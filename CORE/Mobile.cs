...
namespace Server
{
	...
	{
		public virtual bool DebugDeserialization{ get{ return false; } }
        ...
        [CommandProperty(AccessLevel.Counselor)]
        public virtual int BluntResistance
        {
            get { return GetResistance(ResistanceType.Blunt); }
        }

        [CommandProperty(AccessLevel.Counselor)]
        public virtual int SlashingResistance
        {
            get { return GetResistance(ResistanceType.Slashing); }
        }

        [CommandProperty(AccessLevel.Counselor)]
        public virtual int PiercingResistance
        {
            get { return GetResistance(ResistanceType.Piercing); }
        }

		public virtual void UpdateResistances()
		{
			if ( m_Resistances == null )
				m_Resistances = new int[8] { int.MinValue, int.MinValue, int.MinValue, int.MinValue, int.MinValue, int.MinValue, int.MinValue, int.MinValue };
		...
		public virtual void ComputeResistances()
		{
            ...
            m_Resistances[5] += this.BaseBluntResistance;
            m_Resistances[6] += this.BaseSlashingResistance;
            m_Resistances[7] += this.BasePiercingResistance;
			...
			for ( int i = 0; i < m_Items.Count; ++i )
			{
                ...
                m_Resistances[5] += item.BluntResistance;
                m_Resistances[6] += item.SlashingResistance;
                m_Resistances[7] += item.PiercingResistance;
			}
			...
			if ( ( m_Map.Rules & MapRules.FreeMovement ) == 0 )
			{
				...
				//{
					...
					else
					{
						...
						if( this.Mounted )
                   		{
                        	...
                        	if ( mount.Stam > 10 && mount.Stam > shoved.Stam )
							{
								mount.Stam -= 10;
						...
						else if ( Stam > 10 && Stam > shoved.Stam )
						{
							Stam -= 10;
		...
		public virtual void Deserialize( GenericReader reader )
		{
			if( DebugDeserialization )
				Console.WriteLine( "Deserializing " + this.ToString() + " " + this.Serial.ToString() + "." );
			 ...
			 * */
			if( DebugDeserialization )
				Console.WriteLine( "Finished deserializing " + this.ToString() + " " + this.Serial.ToString() + "." );
		}
		...
		public void AddItem( Item item )
		{
			...
			if ( item.PhysicalResistance != 0 || item.FireResistance != 0 || item.ColdResistance != 0 ||
                item.PoisonResistance != 0 || item.EnergyResistance != 0 || item.BluntResistance != 0 ||
                item.SlashingResistance != 0 || item.PiercingResistance != 0)
				UpdateResistances();
		}
		...
		private MobileDelta GetStatDelta( StatType type )
		{
			...
			if ( ( type & StatType.HitsMax ) != 0 )
				delta |= MobileDelta.Hits;

			if ( ( type & StatType.StamMax ) != 0 )
				delta |= MobileDelta.Stam;

			if ( ( type & StatType.ManaMax ) != 0 )
        ...
        public virtual void OnRawHitsChange(int oldValue)
        {
        }

        public virtual void OnRawStamChange(int oldValue)
        {
        }

        public virtual void OnRawManaChange(int oldValue)
        {
        }
		...
		public int RawHits
		{
			...
			set
			{
				...
				if ( m_HitsMax != value )
				{
					...
					OnRawHitsChange( oldValue );
					OnRawStatChange( StatType.HitsMax, oldValue );
				}
			}
		}
		...
		public int HitsMax
		{
            get
            {
                int value = m_HitsMax + GetStatOffset( StatType.HitsMax );
		...
		public int RawStam
		{
			...
			set
			{
				...
				if ( m_StamMax != value )
				{
					...
					OnRawDexChange( oldValue );
					OnRawStatChange( StatType.StamMax, oldValue );
				}
			}
		}
        ...
        public int StamMax
        {
            get
            {
                int value = m_StamMax + GetStatOffset( StatType.StamMax );
		...
		public int RawMana
		{
			...
			set
			{
				...
				if ( m_ManaMax != value )
				{
					...
					OnRawIntChange( oldValue );
					OnRawStatChange( StatType.ManaMax, oldValue );
				}
			}
		}
		...
		public int ManaMax
		{
            get
            {
                int value = m_ManaMax + GetStatOffset( StatType.ManaMax );
		...
		public virtual bool CanSee( Mobile m )
		{
			...
			return this == m || (
				...
				( ( m.Alive || ( Core.SE && Skills.Linguistics.Value >= 100.0 ) ) || !Alive || m_AccessLevel > AccessLevel.Player || m.Warmode ) );
...
//		public int HairSerial { get { return HairInfo.FakeSerial( this ); } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public virtual int FacialHairItemID
		{
			get 
			{ 
				if( m_FacialHair == null ) 
...
//		public int FacialHairSerial { get { return FacialHairInfo.FakeSerial( this ); } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public virtual int FacialHairHue {
			get 
			{
        ...
        private int m_HairHueMod = -1;
        private int m_FacialHairHueMod = -1;

        [CommandProperty( AccessLevel.GameMaster )]
        public virtual int HairHueMod
        {
            get
            {
                return m_HairHueMod;
        ...
        [CommandProperty( AccessLevel.GameMaster )]
        public virtual int FacialHairHueMod
        {
            get
            {
                return m_FacialHairHueMod;
            }
            set
            {
                if( m_FacialHairHueMod != value )
                {
                    m_FacialHairHueMod = value;
                    Delta( MobileDelta.FacialHair );
                }
            }
        }
...
*/		}
		...
		public int RawStatTotal
		{
			get
			{
				return RawStr + RawDex + RawInt + RawHits + RawStam + RawMana;
			}
		}
