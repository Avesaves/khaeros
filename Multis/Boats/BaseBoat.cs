...
namespace Server.Multis
{
	...
	public abstract class BaseBoat : BaseMulti
	{
        ...
        public virtual bool CanBeDryDocked() { return false; }
		...
		public bool CanCommand( Mobile m )
		{
			...
			if( (this is SmallBoat || this is SmallDragonBoat) && pm.Feats.GetFeatLevel(FeatList.Cartography) > 0 )
				return true;
			
			if( (this is MediumBoat || this is MediumDragonBoat) && pm.Feats.GetFeatLevel(FeatList.Cartography) > 1 )
				return true;

            if ((this is LargeBoat || this is LargeDragonBoat || this is DryDockableBoat) && pm.Feats.GetFeatLevel(FeatList.Cartography) > 2)
				return true;
			
			m.SendMessage( "You lack the appropriate level of the Cartography skill." );
			return false;
		}
        ...
        public enum DryDockResult { Valid, Cant, Dead, NoKey, NotAnchored, Mobiles, Items, Hold }

        public DryDockResult CheckDryDock(Mobile from)
        {

            if (CanBeDryDocked() == false)
                return DryDockResult.Cant;
