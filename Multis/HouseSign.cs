...
namespace Server.Multis
{
	public class HouseSign : Item
	{
        ...
        private TownHouseSign m_TownHouseSign;

        [CommandProperty( AccessLevel.GameMaster )]
        public TownHouseSign TownHouseSign
        {
            get 
            {
                if( m_TownHouseSign == null )
                {
                    for( int i = 0; i < TownHouseSign.AllSigns.Count; i++ )
                    {
                        TownHouseSign sign = TownHouseSign.AllSigns[i] as TownHouseSign;

                        if( sign.House != null && sign.House.Sign == this )
                        {
                            m_TownHouseSign = sign;
                            break;
                        }
                    }
                }

                return m_TownHouseSign; 
            }
            set { m_TownHouseSign = value; }
        }
		...
		public string OriginalName
		{
			...
			set{ m_OriginalName = value; InvalidateProperties(); }
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			else if( OriginalName != null && OriginalName.Length > 0 )
				name = OriginalName;
			
			else
				name = m_Owner.Owner.RawName;
			
			list.Add( 1061640, name ); // Owner: ~1_OWNER~
			...
			if ( m_Owner != null )
			{
                ...
                if( TownHouseSign != null )
                    list.Add( 1060659, "Seasonal Tax\t" + TownHouseSign.Price.ToString() );
			}
		}

		public override void OnSingleClick( Mobile from )
		{
			/*if ( m_Owner != null && m_Owner.DecayPeriod != TimeSpan.Zero && BaseHouse.DecayEnabled )
			{
				string message;
