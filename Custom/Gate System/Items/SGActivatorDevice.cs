...
namespace Server.SG
{
    public class SGActivatorDevice : Item
    {
        ...
        [Constructable]
        public SGActivatorDevice(int sgaxloc, int sgayloc, int sgazloc, string sgafacing, int sgastyle, bool canbeused, bool beingused, bool energy, bool hidden, string discover, string locname, int facetID, int sgac1, int sgac2, int sgac3, int sgac4, int sgac5) : base(10906)
        {
            m_SGAXloc = sgaxloc;
            m_SGAYloc = sgayloc;
            m_SGAZloc = sgazloc;
        ...
        public override void OnDoubleClick(Mobile from)
        {
			...
			Item bookpack = pack.FindItemByType( typeof( CustomSpellBook ));
			Item checkforheldbook = from.FindItemOnLayer( Layer.FirstValid ); 
				
            if (checkforheldbook != null && checkforheldbook is CustomSpellBook || bookpack != null)
            {
                ...
                {
                    from.SendMessage(33, "This altar doesn't appear to do anything.");
                }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (m_TimerCounter == 1)
                                {
                                    from.SendMessage(33, "You place your spellbook upon the altar.");
                                    this.Hue = 0;
             ...
             from.SendMessage(33, "You can't quite figure out what this is used for...");
        ...
        private class SGActivatorDeviceTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                if (m_sgactivatordevice.m_SGABeingUsed)
                {
                    ...
                    if (PulseTimer >= 60)
                    {
                        foreach (NetState state in NetState.Instances)
                        {
                            ...
                            if (mob.Name == m_sgactivatordevice.m_WhoClickedIt)
                            {
                                ...
                                {
                                    mob.SendMessage(33, "The altar becomes inactive after {0} seconds if not being used.", (SGCore.SGSystemBlink * PulseTimer).ToString());
                                }
                                else
                                {
                                    mob.SendMessage(33, "The altar becomes inactive after {0} minutes if not being used.", ( (SGCore.SGSystemBlink * PulseTimer) / 60).ToString());
                                }
                            }
                        }
                    ...
                    if (m_sgactivatordevice.Hue == 0)
                    {
                        m_sgactivatordevice.Hue = 0;
                    }
                    else if (m_sgactivatordevice.Hue == 0)
                    {
                        m_sgactivatordevice.Hue = 0;
                    }
                    else if (m_sgactivatordevice.Hue == 0)
