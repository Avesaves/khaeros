		...
		public virtual void Explode( Mobile from )
		{
			...
			for( int i = 0; i < list.Count; i++ )
			{
                ...
                if (m is PlayerMobile)
				    AOS.Damage( m, Caster, Convert.ToInt32(ExplosionDamage*(Caster.RawInt*.01)), false, 0, 0, 0, 0, 100, 0, 0, 0, false );
                else
                    AOS.Damage(m, Caster, Convert.ToInt32((ExplosionDamage*2)*(Caster.RawInt*.01)), false, 0, 0, 0, 0, 100, 0, 0, 0, false);
			}
		}
