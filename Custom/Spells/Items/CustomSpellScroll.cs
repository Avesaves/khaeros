...
namespace Server.Items
{
    ...
    {
        CustomMageSpell m_Spell = new CustomMageSpell(null, 1);
        ...
        class MenuEntry : ContextMenuEntry
        {
            CustomSpellScroll m_Scroll;
            PlayerMobile m_From;
            ...
            {
                if (m_From == null || m_Scroll == null || m_From.Deleted || m_Scroll.Deleted || !m_From.Alive ||
                    m_From.Paralyzed)
                    return;
                ...
                {
                    m_From.SendMessage(
                        "Despite your magical training, you can't seem to figure out how to use this correctly.");
                    return;
                }
        ...
        {
            if (m.Feats.GetFeatLevel(FeatList.DamagingEffect) < 3 &&
                spell.Damage > (m.Feats.GetFeatLevel(FeatList.DamagingEffect)*25))
                ...
                m.SendMessage(
                    "You lack the appropriate knowledge to create a spell with an explosion area of that magnitude.");

            else if (m.Feats.GetFeatLevel(FeatList.ExplosiveEffect) < 3 &&
                     spell.ExplosionDamage > (m.Feats.GetFeatLevel(FeatList.ExplosiveEffect)*25))
                m.SendMessage(
                    "You lack the appropriate knowledge to create a spell that deals that much damage from explosion.");
            ...
            else if ((m.Feats.GetFeatLevel(FeatList.ChainEffect) < 3 &&
                      spell.ChainedDamage > (m.Feats.GetFeatLevel(FeatList.ChainEffect)*25)))
                m.SendMessage(
                    "You lack the appropriate knowledge to create a spell that deals that much chained damage.");
                ...
                m.SendMessage(
                    "You lack the appropriate knowledge to create a spell whose status lasts for that long.");

            else if ((m.Feats.GetFeatLevel(FeatList.RecurrentEffect) < 3 &&
                      spell.RepDamage > (m.Feats.GetFeatLevel(FeatList.RecurrentEffect)*25)))
                m.SendMessage(
                    "You lack the appropriate knowledge to create a spell that deals that much recurrent damage.");

            else if (spell.Reps > (m.Feats.GetFeatLevel(FeatList.RecurrentEffect)*2))
                m.SendMessage(
                    "You lack the appropriate knowledge to create a spell with that many repetitions.");

            else if ((spell.RepDelay < 11 && m.Feats.GetFeatLevel(FeatList.RecurrentEffect) < 2) ||
                     (spell.RepDelay < 6 && m.Feats.GetFeatLevel(FeatList.RecurrentEffect) < 3))
                m.SendMessage(
                    "You lack the appropriate knowledge to create a spell whose recurrent effect happens so fast.");

            else if (!m.Feats.FeatDictionary.ContainsKey(spell.RequiredFeat))
                m.SendMessage(String.Format("You lack the {0} feat to create this spell.",
                                            spell.RequiredFeat));
        ...
        public override bool OnDroppedOnto(Mobile from, Item target)
        {
            if (target != null && !target.Deleted && target is CustomSpellBook && target.IsChildOf(from))
            {
                ...
                if (Spell.CustomName == null || Spell.CustomName.Length < 1)
                    from.SendMessage(
                        "A spell scroll can only be copied onto a spell book after the spell has been properly named.");

                else if (book.ContainsSpell(Spell.CustomName))
                    from.SendMessage(
                        "That spell book already contains a spell with the same name as the one on the scroll.");
        ...
        public static bool IsMageCheck(Mobile m, bool message)
        {
            if (m is PlayerMobile && ((PlayerMobile) m).Feats.GetFeatLevel(FeatList.Magery) < 1)
            {
                if (message)
                    m.SendMessage(
                        "You cannot decipher anything from this parchment, which is covered in peculiar runes and odd drawings.");
        ...
        public CustomSpellScroll(Serial serial)
            : base(serial)
        {
        }
        ...
        public static CustomMageSpell DeserializeSpell(GenericReader reader)
        {
            ...
            if (version > 0)
                newSpell =
                    (CustomMageSpell) Activator.CreateInstance(ScriptCompiler.FindTypeByName(reader.ReadString(), true));
