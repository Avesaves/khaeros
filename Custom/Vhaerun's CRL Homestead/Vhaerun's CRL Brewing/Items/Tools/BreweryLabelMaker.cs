			...
			protected override void OnTarget( Mobile from, object target )
			{
				...
				else if ( target is Item )
				{
					...
					if ( target == m_LabelMaker || target is BaseCraftMead || target is MeadKeg ||
                         target is BaseCraftAle || target is AleKeg || target is BaseCraftCider || target is CiderKeg)
					{
						...
						else
						{
                            ...
                            else if( item is BaseCraftAle )
                            {
                                BaseCraftAle ale = (BaseCraftAle)item;
                                if( ale.Crafter != from )
                                {
                                    from.SendMessage( "That bottle is either not worth labeling or was not crafted at your brewery!" );
                                }
                            ...
                            else if( item is AleKeg )
                            {
                                AleKeg keg = (AleKeg)item;
                                if( keg.Crafter != from )
                                {
                                    from.SendMessage( "That keg is either not worth labeling or was not crafted at your brewery!" );
                                }
                            ...
                            else if( item is BaseCraftCider )
                            {
                                BaseCraftCider cider = (BaseCraftCider)item;
                                if( cider.Crafter != from )
                                {
                                    from.SendMessage( "That bottle is either not worth labeling or was not crafted at your brewery!" );
                                }
                                else
                            ...
                            else if( item is CiderKeg )
                            {
                                CiderKeg keg = (CiderKeg)item;
                                if( keg.Crafter != from )
                                {
                                    from.SendMessage( "That keg is either not worth labeling or was not crafted at your brewery!" );
                                }
					...
					{
						from.SendMessage( "Invalid Target.   Only crafted mead/ale/cider kegs and bottles, or this label maker can be labeled." );
					}
				}
				else
				{
