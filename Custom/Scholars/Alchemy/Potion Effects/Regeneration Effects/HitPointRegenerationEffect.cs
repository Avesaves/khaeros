...
{
	public class HitPointRegenerationEffect : CustomPotionEffect
	{
		...
		private static void ReleaseHPRegenLock( object state )
		{
			((Mobile)state).EndAction( typeof( HitPointRegenerationEffect ) );
		}

		public override string Name{ get{ return "Hit Point Regeneration"; } }
		...
		public override void ApplyEffect( Mobile to, Mobile source, int intensity, Item itemSource )
		{
			...
			if ( intensity > 0 )
			{
				if ( to.CanBeginAction( typeof( HitPointRegenerationEffect ) ) )
				{
					to.BeginAction( typeof( HitPointRegenerationEffect ) );
					Timer.DelayCall( TimeSpan.FromSeconds( 20 ), new TimerStateCallback( ReleaseHPRegenLock ), to ); // 20 sec delay
				}
		...
		{
			return mobile.CanBeginAction( typeof( HitPointRegenerationEffect ) );
		}
		...
		private class RegenTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if ( m_Harmful )
					m_Mobile.Damage( PointsPerSecond );
				else
					m_Mobile.Hits += PointsPerSecond;
