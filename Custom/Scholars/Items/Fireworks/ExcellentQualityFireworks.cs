...
{
    public class ExcellentQualityFireworks : Item, IEasyCraft
	{
		private FireworksTimer m_Timer;
		
		[Constructable]
		public ExcellentQualityFireworks() : base( 3703 )
		{
			Name = "excellent quality fireworks";
			Hue = 38;
			Weight = 15.0;
		}

		public ExcellentQualityFireworks( Serial serial ) : base( serial )
		{
		}
		...
		{
			private ExcellentQualityFireworks m_Owner;
			private int m_Ticks;
			private bool m_IgniteCountdown;

			public FireworksTimer( ExcellentQualityFireworks owner ) : base( TimeSpan.FromSeconds( 3.0 ), TimeSpan.FromSeconds( 1.0 ) )
			{
			...
			protected override void OnTick()
			{
				...
				if ( m_IgniteCountdown)
				{
					if ( m_Ticks > 3 )
					{
						...
						Delay = TimeSpan.FromSeconds( 90.0 );
