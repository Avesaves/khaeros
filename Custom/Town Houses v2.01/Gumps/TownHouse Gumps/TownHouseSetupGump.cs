...
namespace Knives.TownHouses
{
	public class TownHouseSetupGump : GumpPlusLight
	{
        ...
        private void QuickPage(int width, ref int y)
        {
			...
			//EDITED
            AddHtml(0, y += 20, width / 2 - 20, "<DIV ALIGN=RIGHT>Lockdowns");
            AddTextField(width / 2 + 20, y, 50, 20, 0x480, 0xBBC, "Lockdowns", c_Sign.Secures.ToString());
            AddButton(width / 2 - 5, y + 3, 0x2716, "Lockdowns", new GumpCallback(Secures));

            AddHtml(0, y += 22, width / 2 - 20, "<DIV ALIGN=RIGHT>Secures");
            AddTextField(width / 2 + 20, y, 50, 20, 0x480, 0xBBC, "Secures", c_Sign.Locks.ToString());
            AddButton(width / 2 - 5, y + 3, 0x2716, "Secures", new GumpCallback(Lockdowns));
