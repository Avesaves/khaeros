        ...
        public static void ApplyDamageToPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            ...
            try
            {
                if (range >= 0 || (triggermob != null && !triggermob.Deleted))
                {
                    ...
                    if (range >= 0)
                    {
                        ...
                        if (rangelist != null)
                        {
                            foreach (Mobile p in rangelist)
                            {
                                ...
                                if (p is PlayerMobile || !playeronly)
                                    AOS.Damage(p, damage, phys, fire, cold, pois, ener, 0, 0, 0);
                            }
                            rangelist.Free();
                        }
                    }
                    ...
                    {
                        // just apply it to the mob who triggered
                        AOS.Damage(triggermob, damage, phys, fire, cold, pois, ener, 0, 0, 0);
            ...
            List<XmlSpawner.SpawnPositionInfo> spawnpositioning, Mobile triggermob, Point3D location, Map map, XmlGumpCallback gumpcallback, out string status_str)
        {
            ...
            if (IsTypeKeyword(typeName))
            {
                ...
                switch (kw)
                {
                    ...
                    case typeKeyword.SETONTHIS:
                        {
                            // the syntax is SETONTHIS[,proptest]/prop/value/prop/value...
                            ...
                            if( arglist.Length > 0 )
                            {
                                ...
                                if( objstr.Length < 2 )
                                    status_str = "missing propstest in SETONTHIS";
                            ...
                            if( invoker != null )
                            {
                                if( proptest != null && CheckPropertyString( null, invoker, proptest, null, out status_str ) )
                                    mayApply = true;
                                else if( proptest == null )
                                    mayApply = true;
