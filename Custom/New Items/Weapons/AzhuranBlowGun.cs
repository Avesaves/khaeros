...
namespace Server.Items
{
	public class BlowGun : Item, IUsesRemaining
	{
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( attachment != null )
				list.Add( 1060847, "{0}\t{1}", "  Contains Poisoned Darts", " " ); // ~1_val~ ~2_val~
		}
		...
		private void OnDartHit( object state )
		{
			...
			AOS.Damage( target, from, Utility.RandomMinMax( 4, 6 ), 100, 0, 0, 0, 0, 0, 0, 0 );
			...
			if ( attachment != null )
			{
				attachment.Poisoner = from;
				attachment.OnOtherwiseInjected( target );
			}
		...
		/*public void Reload( Mobile from, BlowGunDarts darts )
		{
			...
			else if ( darts.UsesRemaining > 0 )
			{
				...
				else
				{
					m_UsesRemaining += need;
					darts.UsesRemaining -= need;
					PoisonedFoodAttachment attachment = XmlAttach.FindAttachment( darts, typeof( PoisonedFoodAttachment ) ) as PoisonedFoodAttachment;
					if ( attachment != null )
					{
						PoisonedFoodAttachment attachment2 = new PoisonedFoodAttachment( attachment.Effects, attachment.PoisonDuration, attachment.PoisonActingSpeed, from );
						XmlAttach.AttachTo( this, attachment2 );
					}
				}
