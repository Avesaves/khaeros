...
namespace Server
{
	public class LightCycle
	{
		...
		{
			if( !from.CanBeginAction(typeof(LightCycle)) )
				return 0;
			...
			if( from is PlayerMobile )
			{
				...
				if( m.VampSight )
					return Math.Min( TimeSystem.TimeEngine.CalculateLightLevel(from), 14 );
				
				if( from.Map == Map.Ilshenar )
					return 24;

                if( XmlAttach.FindAttachment( from, typeof( XmlMageSight ) ) != null )
                    return 0;
			}
