...
namespace Server.Misc
{
	public class NotorietyHandlers
	{
		public static void Initialize()
		{
			...
			Notoriety.Hues[Notoriety.Murderer]		= 0xb47;
		...
		public static int MobileNotoriety( Mobile source, Mobile target )
		{
            ...
            if( source is PlayerMobile && target is PlayerMobile && ( (PlayerMobile)target ).IsHardcore )
                return Notoriety.Murderer;

            if (source is PlayerMobile && (target is BaseCreature && (target as BaseCreature).Deadly > 0))
                return Notoriety.Murderer;
