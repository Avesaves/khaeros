	...
	public class Titles
	{
		...
		public static string ComputeTitle( Mobile beholder, Mobile beheld )
		{
			...
			if( beheld is PlayerMobile && ((PlayerMobile)beheld).DisplayChampionTitle )
			{
				...
				if( info.Ghost > 0 )
					title.AppendFormat( ": {0} of Evil", HarrowerTitles[Math.Min( HarrowerTitles.Length, info.Ghost )-1] );
				else
				{
			...
			{
				PlayerMobile m = beheld as PlayerMobile;
				
				if( m.TitlePrefix != null )
				{
					title.Append( m.TitlePrefix );
					title.AppendFormat( " {0}", m.Name );
				}
            ...
            if (beheld is BaseCreature)
            {
                if (beheld.Title != null)
                {
                    title.AppendFormat(", {0}", beheld.Title);
                }
            }
			...
			if ( showSkillTitle && beheld.Player )
			{
				...
				if( beheld is PlayerMobile )
				{
					...
					if( m.RPTitle != null )
					{
						title.AppendFormat( ", {0}", m.RPTitle );
					}
				}
			}
		...
		private static int GetTableType( Skill skill )
		{
			...
			{
				default: return 0;
				case SkillName.Leadership: return 1;
				case SkillName.ExoticWeaponry: return 2;
			}
		}
