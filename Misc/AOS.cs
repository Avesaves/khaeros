...
namespace Server
{
	public class AOS
	{
		...
		public static int Damage( Mobile m, int damage, bool ignoreArmor, int phys, int fire, int cold, int pois, int nrgy, int blut, int slax, int pirc )
		{
			return Damage( m, null, damage, ignoreArmor, phys, fire, cold, pois, nrgy, blut, slax, pirc );
		}

		public static int Damage( Mobile m, int damage, int phys, int fire, int cold, int pois, int nrgy, int blut, int slax, int pirc )
		{
			return Damage( m, null, damage, phys, fire, cold, pois, nrgy, blut, slax, pirc );
		}

		public static int Damage( Mobile m, Mobile from, int damage, int phys, int fire, int cold, int pois, int nrgy, int blut, int slax, int pirc )
		{
			return Damage( m, from, damage, false, phys, fire, cold, pois, nrgy, blut, slax, pirc, false );
		}

		public static int Damage( Mobile m, Mobile from, int damage, bool ignoreArmor, int phys, int fire, int cold, int pois, int nrgy, int blut, int slax, int pirc )
		{
			return Damage( m, from, damage, ignoreArmor, phys, fire, cold, pois, nrgy, blut, slax, pirc, false );
		}

		public static int Damage( Mobile m, Mobile from, int damage, int phys, int fire, int cold, int pois, int nrgy, int blut, int slax, int pirc, bool keepAlive )
		{
			return Damage( m, from, damage, false, phys, fire, cold, pois, nrgy, blut, slax, pirc, keepAlive );
		}

		public static int Damage( Mobile m, Mobile from, int damage, bool ignoreArmor, int phys, int fire, int cold, int pois, int nrgy, int blut, int slax, int pirc, bool keepAlive )
		{
			...
			if( !ignoreArmor )
			{
				...
				if ( m is Mercenary )
					resPhys = Math.Min( Mobile.MaxPlayerResistance, resPhys );
				int resFire = m.FireResistance;
				if ( m is Mercenary )
					resFire = Math.Min( Mobile.MaxPlayerResistance, resFire );
				int resCold = m.ColdResistance;
				if ( m is Mercenary )
					resCold = Math.Min( Mobile.MaxPlayerResistance, resCold );
				int resPois = m.PoisonResistance;
				if ( m is Mercenary )
					resPois = Math.Min( Mobile.MaxPlayerResistance, resPois );
				int resNrgy = m.EnergyResistance;
				if ( m is Mercenary )
					resNrgy = Math.Min( Mobile.MaxPlayerResistance, resNrgy );
				int resBlut = m.BluntResistance;
				if ( m is Mercenary )
					resBlut = Math.Min( Mobile.MaxPlayerResistance, resBlut );
				int resSlax = m.SlashingResistance;
				if ( m is Mercenary )
					resSlax = Math.Min( Mobile.MaxPlayerResistance, resSlax );
				int resPirc = m.PiercingResistance;
				if ( m is Mercenary )
					resPirc = Math.Min( Mobile.MaxPlayerResistance, resPirc );
				...
				totalDamage += damage * blut * (100 - resBlut);
				totalDamage += damage * slax * (100 - resSlax);
				totalDamage += damage * pirc * (100 - resPirc);
	...
	public enum AosAttribute
	{
        ...
        BonusHitsMax=0x00800000,
        BonusStamMax=0x01000000,
        BonusManaMax=0x02000000
	}
	...
	public sealed class AosAttributes : BaseAttributes
	{
		...
		public static int GetValue( Mobile m, AosAttribute attribute )
		{
            ...
            value += XmlAosAttribute.GetValue(m, attribute);
			...
			for( int i = 0; i < items.Count; ++i )
			{
                ...
                else if( obj is ArmourBackpack )
                {
                    AosAttributes attrs = ( (ArmourBackpack)obj ).Attributes;

                    if( attrs != null )
                        value += attrs[attribute];

                    if( attribute == AosAttribute.Luck )
                        value += ( (ArmourBackpack)obj ).GetLuckBonus();
                }
        ...
        [CommandProperty( AccessLevel.GameMaster )]
        public int BonusHitsMax { get { return this[AosAttribute.BonusHitsMax]; } set { this[AosAttribute.BonusHitsMax] = value; } }

        [CommandProperty( AccessLevel.GameMaster )]
        public int BonusStamMax { get { return this[AosAttribute.BonusStamMax]; } set { this[AosAttribute.BonusStamMax] = value; } }

        [CommandProperty( AccessLevel.GameMaster )]
        public int BonusManaMax { get { return this[AosAttribute.BonusManaMax]; } set { this[AosAttribute.BonusManaMax] = value; } }
	...
	public enum AosWeaponAttribute
	{
        ...
        ResistBluntBonus=0x02000000,
        ResistSlashingBonus=0x04000000,
        ResistPiercingBonus=0x08000000
	}
	...
	public sealed class AosWeaponAttributes : BaseAttributes
	{
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public int ResistBluntBonus { get { return this[AosWeaponAttribute.ResistBluntBonus]; } set { this[AosWeaponAttribute.ResistBluntBonus] = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public int ResistSlashingBonus { get { return this[AosWeaponAttribute.ResistSlashingBonus]; } set { this[AosWeaponAttribute.ResistSlashingBonus] = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public int ResistPiercingBonus { get { return this[AosWeaponAttribute.ResistPiercingBonus]; } set { this[AosWeaponAttribute.ResistPiercingBonus] = value; } }
	...
	public sealed class AosElementAttributes : BaseAttributes
	{
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int Blunt { get { return this[AosElementAttribute.Blunt]; } set { this[AosElementAttribute.Blunt] = value; } }

		[CommandProperty( AccessLevel.GameMaster )]
		public int Slashing { get { return this[AosElementAttribute.Slashing]; } set { this[AosElementAttribute.Slashing] = value; } }

		[CommandProperty( AccessLevel.GameMaster )]
		public int Piercing { get { return this[AosElementAttribute.Piercing]; } set { this[AosElementAttribute.Piercing] = value; } }
