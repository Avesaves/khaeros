...
namespace Server.Mobiles
{
	public class AnimalAI : BaseAI
	{
		...
		public override bool DoActionWander()
		{
			...
			if (AcquireFocusMob(m_Mobile.RangePerception, FightMode.FindThreat, false, false, false))
			{
				m_Mobile.DebugSay( "There is something suspicious near, I go away" );
				Action = ActionType.Backoff;
			}
			else if ( !( m_Mobile is IEnraged ) && ( m_Mobile.IsHurt() || m_Mobile.Combatant != null ) )
			{
...
/*
			...
			if ( !( m_Mobile is IEnraged ) && !m_Mobile.Summoned && !m_Mobile.Controlled && hitPercent < 0.1 ) // Less than 10% health
			{
		...
		public override bool DoActionCombat()
		{
			...
			if ( !( m_Mobile is IEnraged ) && !m_Mobile.Controlled && !m_Mobile.Summoned )
			{
				double hitPercent = (double)m_Mobile.Hits / m_Mobile.HitsMax;
