	...
	public class MageAI : BaseAI
	{
		...
		public override bool DoActionWander()
		{
			if ( AcquireFocusMob( m_Mobile.RangePerception, m_Mobile.FightMode, false, false, true ) )
			{
				...
				{
					if( m_Mobile is Petal )
						m_Mobile.Emote( "*flies out of a tree's foliage to attack " + m_Mobile.Combatant.Name + "*" );
				}
			}
		...
		public override bool DoActionCombat()
		{
			...
			if ( SmartAI && !m_Mobile.StunReady && m_Mobile.Skills[SkillName.UnarmedFighting].Value >= 80.0 && m_Mobile.Skills[SkillName.Anatomy].Value >= 80.0 )
			...
			if ( m_Mobile.Mana > 9 && c != null && m_Mobile.InRange( c, 6 ) )
				RunFrom( c );
			else if ( c != null && !m_Mobile.InRange( c, 8 ) && m_Mobile.InRange( c, 16 ) )
				RunTo( c );
			else if ( c != null && m_Mobile.Mana < 10 )
				RunTo( c );
		...
		private bool ProcessTarget()
		{
			...
			if ( isDispel )
			{
				...
				if ( m_Mobile.Mana > 9 && toTarget != null && m_Mobile.InRange( toTarget, 6 ) )
					RunFrom( toTarget );
				else if ( toTarget != null && !m_Mobile.InRange( toTarget, 8 ) && m_Mobile.InRange( toTarget, 16 ) )
					RunTo( toTarget );
				else if ( toTarget != null && m_Mobile.Mana < 10 )
					RunTo( toTarget );
			}
