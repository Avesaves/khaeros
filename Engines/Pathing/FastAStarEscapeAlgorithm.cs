...
namespace Server.PathAlgorithms.FastAStarEscape
{
	...
	public class FastAStarEscapeAlgorithm : PathAlgorithm
	{
		public static PathAlgorithm Instance = new FastAStarEscapeAlgorithm();
		...
		public bool CheckEscapeCondition( Mobile m, Map map, Point3D start, int escapeRange )
		{
			return escapeRange <= AreaSize;
		}
		...
		public Direction[] FindEscapePath( Mobile m, Map map, Point3D start, Point3D goal, int goalDist )
		{
			if ( goalDist > AreaSize )
				return null;
					...
					if ( !wasTouched )
					{
						...
						if ( !wasTouched || m_Nodes[newNode].total > newTotal )
						{
							...
							if ( !wasTouched || !m_OnOpen[newNode] )
							{
								...
								if ( GetDist( newNode, destNode ) >= goalDist )
								{
									pathCount = 0;
		...
		private int GetDist( int node1, int node2 )
		{
			int x1 = node1 % AreaSize;
			int y1 = (node1 / AreaSize) % AreaSize;
			int z1 = m_Nodes[node1].z;
			int x2 = node2 % AreaSize;
			int y2 = (node2 / AreaSize) % AreaSize;
			int z2 = m_Nodes[node2].z;
