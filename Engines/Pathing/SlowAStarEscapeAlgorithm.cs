...
namespace Server.PathAlgorithms.SlowAStarEscape
{
	...
	public class SlowAStarEscapeAlgorithm : PathAlgorithm
	{
		public static PathAlgorithm Instance = new SlowAStarEscapeAlgorithm();
		...
		private int GetDist( int x1, int y1, int z1, int x2, int y2, int z2 )
		{
		...
		public Direction[] FindEscapePath( Mobile m, Map map, Point3D start, Point3D goal, int goalDist )
		{
			...
			while ( openCount > 0 )
			{
				...
				if ( GetDist( curNode.x, curNode.y, curNode.z, goalNode.x, goalNode.y, goalNode.z ) >= goalDist )
				{
