...
namespace Server.Movement
{
	...
	{
		private const int PersonHeight = 16; // tiles need to be at 15 or less to pass thru
		private const int MountOffset = 7; // tiles need to be at 22 or less to pass thru
		...
		private bool Check( Map map, Mobile m, List<Item> items, int x, int y, int startTop, int startZ, bool canSwim, bool cantWalk, out int newZ )
		{
			...
			int checkTop = startZ + PersonHeight + ( m.Mounted ? MountOffset : 0 );
			...
			for ( int i = 0; i < tiles.Length; ++i )
			{
				...
				if ( (flags & ImpassableSurface) == TileFlag.Surface || (canSwim && (flags & TileFlag.Wet) != 0) ) // Surface && !Impassable
				{
					...
					int ourTop = ourZ + PersonHeight + ( m.Mounted ? MountOffset : 0 );
					...
					if ( ourZ + PersonHeight + ( m.Mounted ? MountOffset : 0 ) > testTop )
						testTop = ourZ + PersonHeight + ( m.Mounted ? MountOffset : 0 );
