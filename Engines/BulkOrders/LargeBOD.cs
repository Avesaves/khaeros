		...
		public void EndCombine( Mobile from, object o )
		{
			if ( o is Item && ((Item)o).IsChildOf( from.Backpack ) )
			{
				if ( o is SmallBOD )
				{
					...
					else if ( m_Material >= BulkMaterialType.Copper && m_Material <= BulkMaterialType.Steel && small.Material != m_Material )
					{
						from.SendLocalizedMessage( 1045162 ); // Both orders must use the same ore type.
					}
					else if ( m_Material >= BulkMaterialType.Thick && m_Material <= BulkMaterialType.Scaled && small.Material != m_Material )
					{
