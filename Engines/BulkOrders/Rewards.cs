...
namespace Server.Engines.BulkOrders
{
	...
	public sealed class SmithRewardCalculator : RewardCalculator
	{
		...
		public override int ComputePoints( int quantity, bool exceptional, BulkMaterialType material, int itemCount, Type type )
		{
			...
			if ( material >= BulkMaterialType.Copper && material <= BulkMaterialType.Steel )
				points += 200 + (50 * (material - BulkMaterialType.Copper));
		...
		public override int ComputeGold( int quantity, bool exceptional, BulkMaterialType material, int itemCount, Type type )
		{
			...
			int mtrlIndex = ( material >= BulkMaterialType.Copper && material <= BulkMaterialType.Steel ) ? 1 + (int)(material - BulkMaterialType.Copper) : 0;
	...
	public sealed class TailorRewardCalculator : RewardCalculator
	{
		...
		public override int ComputePoints( int quantity, bool exceptional, BulkMaterialType material, int itemCount, Type type )
		{
			...
			if ( material == BulkMaterialType.Thick )
				points += 50;
			else if ( material == BulkMaterialType.Beast )
				points += 100;
			else if ( material == BulkMaterialType.Scaled )
				points += 150;
		...
		public override int ComputeGold( int quantity, bool exceptional, BulkMaterialType material, int itemCount, Type type )
		{
			...
			int quanIndex = ( quantity == 20 ? 2 : quantity == 15 ? 1 : 0 );
			int mtrlIndex = ( material == BulkMaterialType.Scaled ? 3 : material == BulkMaterialType.Beast ? 2 : material == BulkMaterialType.Thick ? 1 : 0 );
