	...
	public abstract class SmallBOD : Item
	{
		...
		public static BulkMaterialType GetMaterial( CraftResource resource )
		{
			switch ( resource )
			{
				...
				case CraftResource.Tin:				return BulkMaterialType.Tin;
				case CraftResource.Obsidian:		return BulkMaterialType.Obsidian;
				case CraftResource.Steel:	    	return BulkMaterialType.Steel;
				case CraftResource.ThickLeather:	return BulkMaterialType.Thick;
				case CraftResource.BeastLeather:	return BulkMaterialType.Beast;
				case CraftResource.ScaledLeather:	return BulkMaterialType.Scaled;
			}
		...
		public void EndCombine( Mobile from, object o )
		{
			if ( o is Item && ((Item)o).IsChildOf( from.Backpack ) )
			{
				...
				else
				{
					...
					if ( m_Material >= BulkMaterialType.Copper && m_Material <= BulkMaterialType.Steel && material != m_Material )
					{
						from.SendLocalizedMessage( 1045168 ); // The item is not made from the requested ore.
					}
					else if ( m_Material >= BulkMaterialType.Thick && m_Material <= BulkMaterialType.Scaled && material != m_Material )
					{
