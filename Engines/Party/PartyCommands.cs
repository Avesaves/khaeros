		...
		public override void OnPrivateMessage( Mobile from, Mobile target, string text )
		{
			...
			if ( p != null && p.Contains( target ) )
			{
				foreach ( NetState state in NetState.Instances )
				{
					...
					if ( staffer != null && staffer.AccessLevel > AccessLevel.Player && staffer is PlayerMobile && ( ( (PlayerMobile)staffer ).HearAll == 2 || ( (PlayerMobile)staffer ).HearAll == 3 ) )
						staffer.SendMessage( 170, "[PARTY]" + from.Name + " to " + target.Name + ": " + text );
				}
		...
		public override void OnPublicMessage( Mobile from, string text )
		{
			...
			if ( p != null )
			{
				foreach ( NetState state in NetState.Instances )
				{
					...
					if ( staffer != null && staffer.AccessLevel > AccessLevel.Player && staffer is PlayerMobile && ( ( (PlayerMobile)staffer ).HearAll == 2 || ( (PlayerMobile)staffer ).HearAll == 3 ) )
						staffer.SendMessage( 170, "[PARTY]" + from.Name + ": " + text );
				}
