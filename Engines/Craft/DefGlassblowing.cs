...
namespace Server.Engines.Craft
{
	public class DefGlassblowing : CraftSystem
	{
		...
		public override int CanCraft( Mobile from, BaseTool tool, Type itemType )
		{
			...
			else if ( !(from is PlayerMobile && ((PlayerMobile)from).Glassblowing) )
				return 1044634; // You havent learned glassblowing.
			else if ( !BaseTool.CheckAccessible( tool, from ) )
				return 1044263; // The tool must be on your person to use.
			else if( m.Feats.GetFeatLevel(FeatList.GlassBlower) < 3 && itemType != null && itemType == typeof( Bottle ) )
		...
		public override void InitCraftList()
		{
			...
			//AddCraft( typeof( GlassPitcher ), 1044050, "glass pitcher", 52.5, 102.5, typeof( Sand ), 1044625, 3, 1044627 );
			AddCraft( typeof( Bottle ), 1044050, "bottle", 52.5, 102.5, typeof( Sand ), 1044625, 2, 1044627 );
			AddCraft( typeof( EmptyPitcher ), 1044050, "glass pitcher", 52.5, 102.5, typeof( Sand ), 1044625, 2, 1044627 );
            ...
            AddCraft( typeof( EmptyWineBottle ), 1044050, "empty wine bottle", 5.0, 10.0, typeof( Sand ), 1044625, 2, 1044627 );
            AddCraft( typeof( EmptyAleBottle ), 1044050, "empty ale bottle", 5.0, 10.0, typeof( Sand ), 1044625, 2, 1044627 );
            AddCraft( typeof( EmptyMeadBottle ), 1044050, "empty mead bottle", 5.0, 10.0, typeof( Sand ), 1044625, 2, 1044627 );
		}
	}
}
