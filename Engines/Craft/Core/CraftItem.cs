...
namespace Server.Engines.Craft
{
	...
	public class CraftItem
	{
		...
		private static Type[][] m_TypesTable = new Type[][]
			{
				...
				new Type[]{ typeof( ThickLeather ), typeof( ThickHides ) },
				new Type[]{ typeof( BeastLeather ), typeof( BeastHides ) },
				new Type[]{ typeof( ScaledLeather ), typeof( ScaledHides ) },
		...
		public bool ConsumeRes( Mobile from, Type typeRes, CraftSystem craftSystem, ref int resHue, ref int maxAmount, ConsumeType consumeType, ref object message, bool isFailure )
		{
			...
			if ( from is PlayerMobile && m.Feats.GetFeatLevel(FeatList.Steel) < 3 && typeRes == typeof( SteelIngot ) )
			{
				message = 1063534; // You do not know how to use that resource.
				return false;
			}
			
			if ( from is PlayerMobile && m.Feats.GetFeatLevel(FeatList.Greenheart) < 3 && typeRes == typeof( GreenheartLog ) )
			{
				message = 1063534; // You do not know how to use that resource.
				return false;
			}
...
/* 			if ( from is PlayerMobile && m.Feats.GetFeatLevel(FeatList.Linen) < 3 && typeRes == typeof( Linen ) )
			...
			if ( typeRes == typeof( ObsidianIngot ) && m_Type.IsSubclassOf( typeof( BaseArmor ) ) )
			{
				message = 1063535; // Obsidian cannot be used to craft armor.
				return false;
			}
			...
			for ( int i = 0; i < types.Length; ++i )
			{
				...
				if ( from is PlayerMobile && m.Feats.GetFeatLevel(FeatList.Bone) < 3 && baseType == typeof( Bone ) )
				{
					message = 1063534; // You do not know how to use that resource.
					return false;
				}
			...
			int index = 0;
			//noaward = false;
            m.AwardedForCurrentCraft = false;
			// Consume ALL
		...
		private void OnResourceConsumed( Item item, int amount )
		{
			...
			if( !UseAllRes )
			{
				if( m.XPFromCrafting && !m.AwardedForCurrentCraft )
				{
					if( ( m.Level < 10 && amount > 1 ) || ( m.Level < 20 && amount > 3 ) || ( m.Level < 30 && amount > 5 ) || ( m.Level < 40 && amount > 7 ) || ( amount > 9 ) )
						LevelSystem.AwardCraftXP( m );
		...
		public double GetExceptionalChance( CraftSystem system, double chance, Mobile from )
		{
			...
			switch ( system.ECA )
			{
				...
				case CraftECA.ChanceMinusSixtyToFourtyFive:
				{
                    ...
                    if( from is PlayerMobile && chance > 0.05 )
                        chance = (0.05 + ((PlayerMobile)from).Feats.GetFeatLevel(FeatList.Craftsmanship) * 0.15);
		...
		public void CompleteCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CustomCraft customCraft )
		{
			...
			if ( CheckSkills( from, typeRes, craftSystem, ref ignored, ref allRequiredSkills ) )
			{
				...
				if ( item != null )
				{
					...
					if( from is PlayerMobile && m.CraftContainer != null && !m.CraftContainer.Deleted && m.Backpack != null && !m.Backpack.Deleted && m.CraftContainer.IsChildOf( m.Backpack ) )
					{
						if( item.Stackable && m.CraftContainer is BaseContainer )
							((BaseContainer)m.CraftContainer).DropAndStack( item );
						
						else
							m.CraftContainer.DropItem( item );
					}
					
					else
					{
						if( item.Stackable && from.Backpack != null && !from.Backpack.Deleted && from.Backpack is BaseContainer )
							((BaseContainer)from.Backpack).DropAndStack( item );
		...
		private class InternalTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					if ( quality == 5 && m_From.Skills[m_CraftSystem.MainSkill].Base >= 100.0 )
