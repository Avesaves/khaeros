...
namespace Server.SkillHandlers
{
	public class Dodge
	{
		...
		{
			SkillInfo.Table[(int)SkillName.Dodge].Callback = new SkillUseCallback( OnUse );
		}
		...
		private class InternalFirstTarget : Target
		{
			...
			public InternalFirstTarget( Mobile from, BaseInstrument instrument ) : base( BaseInstrument.GetBardRange( from, SkillName.Dodge ), false, TargetFlags.None )
			{
		...
		private class InternalSecondTarget : Target
		{
			...
			public InternalSecondTarget( Mobile from, BaseInstrument instrument, BaseCreature creature ) : base( BaseInstrument.GetBardRange( from, SkillName.Dodge ), false, TargetFlags.None )
			{
				m_Instrument = instrument;
				m_Creature = creature;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted is BaseCreature )
				{
					...
					else if ( m_Creature.Map != creature.Map || !m_Creature.InRange( creature, BaseInstrument.GetBardRange( from, SkillName.Dodge ) ) )
					{
					...
					else if ( m_Creature != creature )
					{
						...
						if ( from.CanBeHarmful( m_Creature, true ) && from.CanBeHarmful( creature, true ) )
						{
							...
							else
							{
								...
								if ( !from.CheckTargetSkill( SkillName.Dodge, creature, diff-25.0, diff+25.0 ) )
								{
									from.NextSkillTime = DateTime.Now + TimeSpan.FromSeconds( 5.0 );
