	...
	public class Anatomy
	{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is BaseCreature && !(targeted as BaseCreature).Controlled && !(targeted is Soldier) )
				{
                ...
                else if ( (targeted is BaseCreature && ((targeted as BaseCreature).Controlled || targeted is Soldier)) || targeted is PlayerMobile)
                {
                    if (targeted is PlayerMobile || (targeted is BaseCreature && ((targeted as BaseCreature).BodyValue == 400 || (targeted as BaseCreature).BodyValue == 401)))
                    {
                        if ((from as PlayerMobile).Feats.GetFeatLevel(FeatList.Anatomy) >= (from == targeted ? 1 : 3))
                        {
                            from.SendGump(new HealthGump(from as PlayerMobile, targeted as Mobile));
                            from.SendMessage("You examine " + (targeted as Mobile).Name + "'s anatomy...");
                            return;
                        }
                        else
                            from.SendMessage("You do not know enough about human anatomy to do that.");
                    }
                    else if (targeted is BaseCreature)
                    {
                        if ((from as PlayerMobile).Feats.GetFeatLevel(FeatList.Veterinary) >= 1)
                        {
                            from.SendGump(new HealthGump(from as PlayerMobile, targeted as Mobile));
                            from.SendMessage("You examine " + (targeted as Mobile).Name + "'s anatomy...");
                            return;
                            ...
                            from.SendMessage("You do not know enough about animal anatomy to do that.");
                    }
                    else
                    {
                        from.SendMessage("You do not know enough about anatomy.");
                        return;
                    }
                }
