					...
					else
					{
						...
						if ( alreadyOwned || m_Tamer.CheckTargetSkill( SkillName.AnimalTaming, m_Creature, minSkill - 25.0, minSkill + 25.0 ) )
						{
							...
							else
							{
								...
								if( m_Tamer is PlayerMobile && ((PlayerMobile)m_Tamer).XPFromCrafting )
									Server.Misc.LevelSystem.AwardCraftXP( (PlayerMobile)m_Tamer, true );
							}

							m_Creature.SetControlMaster( m_Tamer );
							m_Creature.IsBonded = false;
							m_Creature.MarkedForTermination = false;
						}
						else
						{
							if( m_Tamer is PlayerMobile && ((PlayerMobile)m_Tamer).XPFromCrafting && m_Tamer.Skills[SkillName.AnimalTaming].Base >= m_Creature.MinTameSkill )
								Server.Misc.LevelSystem.AwardCraftXP( (PlayerMobile)m_Tamer, true );
