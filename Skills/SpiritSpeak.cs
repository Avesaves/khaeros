...
namespace Server.SkillHandlers
{
	class SpiritSpeak
	{
		...
		public static TimeSpan OnUse( Mobile m )
		{
			...
			if ( m.CheckSkill( SkillName.Linguistics, 0, 100 ) )
			{	
				if ( !m.CanHearGhosts )
				{
					Timer t = new SpiritSpeakTimer( m );
					double secs = m.Skills[SkillName.Linguistics].Base / 50;
					secs *= 90;
					if ( secs < 15 )
						secs = 15;
		...
		private class SpiritSpeakSpell : Spell
		{
			...
			public override void OnCast()
			{
				...
				if ( toChannel != null )
				{
					min = 1 + (int)(Caster.Skills[SkillName.Linguistics].Value * 0.25);
					max = min + 4;
					mana = 0;
				...
				{
					Caster.CheckSkill( SkillName.Linguistics, 0.0, 120.0 );

					if ( Utility.RandomDouble() > (Caster.Skills[SkillName.Linguistics].Value / 100.0) )
					{
