...
namespace Server.SkillHandlers
{
	public class Throwing
	{
		...
		{
			SkillInfo.Table[(int)SkillName.Throwing].Callback = new SkillUseCallback( OnUse );
		}
		...
		private class ThrowingInfo
		{
			...
			public ThrowingInfo( Mobile from, Mobile creature, TimeSpan duration, int effect, ArrayList mods )
			{
				m_From = from;
				m_Creature = creature;
		...
		{
			ThrowingInfo info = m_Table[targ] as ThrowingInfo;
		...
		private static void ProcessThrowing( object state )
		{
			ThrowingInfo info = (ThrowingInfo)state;
		...
		public class ThrowingTarget : Target
		{
			private BaseInstrument m_Instrument;

			public ThrowingTarget( Mobile from, BaseInstrument inst ) : base( BaseInstrument.GetBardRange( from, SkillName.Throwing ), false, TargetFlags.Harmful )
			{
				m_Instrument = inst;
			}
			...
			protected override void OnTarget( Mobile from, object target )
			{
				...
				else if ( target is Mobile )
				{
					...
					{
						TimeSpan len = TimeSpan.FromSeconds( from.Skills[SkillName.Throwing].Value * 2 );
						...
						else if ( from.CheckTargetSkill( SkillName.Throwing, target, diff-25.0, diff+25.0 ) )
						{
							if ( !m_Table.Contains( targ ) )
							{
								...
								if ( Core.AOS )
								{
									double discord = from.Skills[SkillName.Throwing].Value;
								...
								{
									effect = (int)( from.Skills[SkillName.Throwing].Value / -5.0 );
									scalar = effect * 0.01;

									mods.Add( new StatMod( StatType.Str, "ThrowingStr", (int)(targ.RawStr * scalar), TimeSpan.Zero ) );
									mods.Add( new StatMod( StatType.Int, "ThrowingInt", (int)(targ.RawInt * scalar), TimeSpan.Zero ) );
									mods.Add( new StatMod( StatType.Dex, "ThrowingDex", (int)(targ.RawDex * scalar), TimeSpan.Zero ) );
								...
								ThrowingInfo info = new ThrowingInfo( from, targ, len, Math.Abs( effect ), mods );
								info.m_Timer = Timer.DelayCall( TimeSpan.Zero, TimeSpan.FromSeconds( 1.25 ), new TimerStateCallback( ProcessThrowing ), info );
