...
*/			
		...
		{
			if ( Core.AOS )
				return TimeSpan.FromSeconds( ( ( 6 * caster.Skills.Invocation.Fixed ) / 50 ) + 1 );
		...
		public static double GetOffsetScalar( Mobile caster, Mobile target, bool curse )
		{
			...
			if( curse )
				percent = 8 + (caster.Skills.Invocation.Fixed / 100) - (target.Skills.MagicResist.Fixed / 100);
			else
				percent = 1 + (caster.Skills.Invocation.Fixed / 100);
		...
		public static int GetOffset( Mobile caster, Mobile target, StatType type, bool curse )
		{
			if ( Core.AOS )
			{
				...
				{
					caster.CheckSkill( SkillName.Invocation, 0.0, 120.0 );
...
/* Tele To */		{ true,			true,			true,			true,			true,				true,				true,			false,					true,					false,				false, 			true,				true	 },
		...
		public static void Damage( Spell spell, TimeSpan delay, Mobile target, Mobile from, double damage, int phys, int fire, int cold, int pois, int nrgy, DFAlgorithm dfa )
		{
			...
			if ( delay == TimeSpan.Zero )
			{
				...
				AOS.Damage( target, from, iDamage, phys, fire, cold, pois, nrgy, 0, 0, 0 );
		...
		private class SpellDamageTimerAOS : Timer
		{
			...
			protected override void OnTick()
			{
				...
				AOS.Damage( m_Target, m_From, m_Damage, m_Phys, m_Fire, m_Cold, m_Pois, m_Nrgy, 0, 0, 0 );
				WeightOverloading.DFA = DFAlgorithm.Standard;
