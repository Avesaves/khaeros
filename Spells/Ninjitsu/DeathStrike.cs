...
namespace Server.Spells.ExoticWeaponry
{
	public class DeathStrike : NinjaMove
	{
		...
		public override void OnHit( Mobile attacker, Mobile defender, int damage )
		{
			...
			if( ExoticWeaponry < 100 ) //This formula is an approximation from OSI data.  TODO: find correct formula
				chance = 30 + (ExoticWeaponry - 85) * 2.2;
			else
				chance = 63 + (ExoticWeaponry - 100) * 1.1;
			...
			if( m_Table.Contains( defender ) )
			{
				...
				if( info.m_Steps > 0 )
					damageBonus = info.m_Attacker.Skills[SkillName.ExoticWeaponry].Fixed / 150;
		...
		private static void ProcessDeathStrike( object state )
		{
			...
			double ExoticWeaponry = info.m_Attacker.Skills[SkillName.ExoticWeaponry].Fixed;
			int divisor = (info.m_Steps >= 5) ? 30 : 80;

			double baseDamage = ExoticWeaponry / divisor;
			...
			AOS.Damage( info.m_Target, info.m_Attacker, damage, true, 100, 0, 0, 0, 0, 0, 0, 0 );
