...
namespace Server.Spells.ExoticWeaponry
{
	public class AnimalForm : NinjaSpell
	{
		...
		public static MorphResult Morph( Mobile m, int entryID )
		{
			...
			if ( m.Skills.ExoticWeaponry.Value < entry.ReqSkill )
			{
				string args = String.Format( "{0}\t{1}\t ", entry.ReqSkill.ToString( "F1" ), SkillName.ExoticWeaponry );
			...
			double ExoticWeaponry = m.Skills.ExoticWeaponry.Value;

			if ( ExoticWeaponry < entry.ReqSkill + 37.5 )
			{
				double chance = (ExoticWeaponry - entry.ReqSkill) / 37.5;

				if ( chance < Utility.RandomDouble() )
					return MorphResult.Fail;
			}

			m.CheckSkill( SkillName.ExoticWeaponry, 0.0, 37.5 );
		...
		public static void AddContext( Mobile m, AnimalFormContext context )
		{
			...
			if ( context.Type == typeof( DisplacerBeast ) || context.Type == typeof( GreyWolf ) )
				m.Hits += 20;
		}
 ...
 * */
		...
		private static AnimalFormEntry[] m_Entries = new AnimalFormEntry[]
			{
				...
				new AnimalFormEntry( typeof( DisplacerBeast ),  1030083, 10083,    0, 1070810,	 82.5, 0xF6, false,  true ),
				new AnimalFormEntry( typeof( BullFrog ),     1028496,  8496, 2003, 1070807,  50.0, 0x51, 0x5A3, false, false ),
				new AnimalFormEntry( typeof( BoaConstrictor ), 1018114,  9663, 2009, 1070808,  50.0, 0x15, false, false ),
		...
		public class AnimalFormGump : Gump
		{
			...
			public AnimalFormGump( Mobile caster, AnimalFormEntry[] entries, AnimalForm spell ) : base( 50, 50 )
			{
				...
				double ExoticWeaponry = caster.Skills.ExoticWeaponry.Value;

				for ( int i = 0; i < entries.Length; ++i )
				{
					bool enabled = ( ExoticWeaponry >= entries[i].ReqSkill );
