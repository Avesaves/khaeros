...
namespace Server.Gumps
{
	...
	public class AdminGump : Gump
	{
		...
		{
			CommandSystem.Register( "Admin", AccessLevel.Seer, new CommandEventHandler( Admin_OnCommand ) );
		}
		...
		public AdminGump( Mobile from, AdminGumpPage pageType, int listPage, ArrayList list, string notice, object state ) : base( 50, 40 )
		{
			...
			bool admin = from.AccessLevel > AccessLevel.GameMaster;
			...
			switch ( pageType )
			{
				...
				case AdminGumpPage.Clients:
				{
					...
					if( !admin )
					{
						for( int i = 0; i < m_List.Count; i++ )
						{
							...
							if( ns.Account != null && ns.Account.AccessLevel > from.AccessLevel )
								m_List.Remove( ns );
						}
					}
				...
				case AdminGumpPage.Accounts:
				{
					...
					if ( rads != null && notice == null && admin )
					{
					...
					if( !admin )
					{
						for( int i = 0; i < m_List.Count; i++ )
						{
							...
							if( a.AccessLevel > from.AccessLevel )
								m_List.Remove( a );
						}
					}
