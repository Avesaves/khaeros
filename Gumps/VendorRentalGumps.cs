...
namespace Server.Gumps
{
	...
	public class VendorRentalContractGump : BaseVendorRentalGump
	{
		...
		{
			from.SendLocalizedMessage( 1062365 ); // Please enter the amount of copper that should be charged for this contract (ESC to cancel):
	...
	public class VendorRentalOfferGump : BaseVendorRentalGump
	{
		...
		protected override void AcceptOffer( Mobile from )
		{
			...
			if ( price > 0 )
			{
				...
				{
					from.SendLocalizedMessage( 1060398, price.ToString() ); // ~1_AMOUNT~ copper has been withdrawn from your bank box.

					int depositedCopper = Banker.DepositUpTo( m_Landlord, price );
					copperToGive = price - depositedCopper;

					if ( depositedCopper > 0 )
						m_Landlord.SendLocalizedMessage( 1060397, price.ToString() ); // ~1_AMOUNT~ copper has been deposited into your bank box.
				...
				{
					from.SendLocalizedMessage( 1062378 ); // You do not have enough copper in your bank account to cover the cost of the contract.
			...
			PlayerVendor vendor = new RentedVendor( from, house, m_Contract.Duration, price, m_Contract.LandlordRenew, copperToGive );
	...
	public class VendorRentalRefundGump : Gump
	{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 )
			{
				...
				{
					m_Landlord.SendLocalizedMessage( 1060398, m_RefundAmount.ToString() ); // ~1_AMOUNT~ copper has been withdrawn from your bank box.

					int depositedCopper = Banker.DepositUpTo( from, m_RefundAmount );

					if ( depositedCopper > 0 )
						from.SendLocalizedMessage( 1060397, depositedCopper.ToString() ); // ~1_AMOUNT~ copper has been deposited into your bank box.

					m_Vendor.HoldCopper += m_RefundAmount - depositedCopper;
