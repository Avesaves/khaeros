...
namespace Server.Gumps
{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 )
			{
				...
				else
				{
                    ...
                    //Checks for tax system
                    if (!m_Vendor.FlatTax && m_Vendor.Taxes > 0)  
                    {
                        double test = m_Vendor.Taxes;
                        test = (test * 0.01);                        
                        int taxedPrice = (int)(leftPrice * test);                        
                        leftPrice -= taxedPrice;
                        if (leftPrice < 0)
                            leftPrice = 0;
                        
                        from.Backpack.ConsumeUpTo(typeof(Copper), taxedPrice);                        
                        Container treasury = m_Vendor.Government.Treasury as Container;
                        treasury.DropItem(new Copper(taxedPrice));                        
                        if (from.Backpack != null)
                        ...
                        m_Vendor.HoldCopper += (m_VI.Price - taxedPrice);
                    }
                    else
                    {
                        if (from.Backpack != null)
                            leftPrice -= from.Backpack.ConsumeUpTo(typeof(Copper), m_VI.Price);
                        if ( leftPrice > 0 && from.BankBox != null )
						    from.BankBox.ConsumeUpTo( typeof( Copper ), leftPrice );
                        m_Vendor.HoldCopper += m_VI.Price;
                    }
					...
					m_Vendor.Log.Add( (from.Name + " purchased " + m_VI.Item.Amount.ToString() + " " + m_VI.Item.Name + " on the " + day + " of " + month + ", " + TimeSystem.Data.Year.ToString() + ".") );
					m_VI.Item.OnPurchased( m_Vendor, from );
				}
			}
			else
			{
	...
	public class PlayerVendorOwnerGump : Gump
	{
		...
		public PlayerVendorOwnerGump( PlayerVendor vendor ) : base( 50, 200 )
		{
			...
			AddHtmlLocalized( 40, 72, 260, 20, 1038321, false, false ); // Copper held for you:
			AddLabel( 300, 72, 0, m_Vendor.HoldCopper.ToString() );
			AddHtmlLocalized( 40, 96, 260, 20, 1038322, false, false ); // Copper held in my account:
			...
			double days = (m_Vendor.HoldCopper + m_Vendor.BankAccount) / ((double)perDay);
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 2:
				{
					from.SendGump( new NPCCustomGump( m_Vendor, ((PlayerMobile)from).Nation, 0, true ) );
	...
	public class NewPlayerVendorOwnerGump : Gump
	{
		...
		public NewPlayerVendorOwnerGump( PlayerVendor vendor ) : base( 50, 200 )
		{
			...
			int copperHeld = vendor.HoldCopper;
			...
			if ( copperHeld < perRealWorldDay )
			{
				int copperNeeded = perRealWorldDay - copperHeld;

				AddHtmlLocalized( 40, 35, 260, 20, 1038320, 0x7FFF, false, false ); // Copper needed for 1 day of vendor salary: 
				AddLabel( 300, 35, 0x1F, copperNeeded.ToString() );
			}
			else
			{
				int days = copperHeld / perRealWorldDay;
			...
			AddHtmlLocalized( 40, 82, 260, 20, 1038322, 0x7FFF, false, false ); // Copper held in my account: 
			AddLabel( 300, 82, 0x480, copperHeld.ToString() );
			...
			AddLabel( 408, 121, 0x480, "Collect Copper" );
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 6: // Collect Copper
				{
					m_Vendor.CollectCopper( from );
	...
	public class PlayerVendorCustomizeGump : Gump
	{
		...
		private static CustomCategory[] Categories = new CustomCategory[]{
			...
			new CustomCategory( Layer.Helm, 1015319, true, new CustomItem[]{//More Hats
				...
				new CustomItem( typeof( TallBrimHat ), 	1011383, 5912 ),
