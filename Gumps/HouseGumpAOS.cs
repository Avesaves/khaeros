...
namespace Server.Gumps
{
	...
	public class HouseGumpAOS : Gump
	{
		...
		private string GetOwnerName()
		{
			...
			if( m_House.Sign.OriginalName != null && m_House.Sign.OriginalName.Length > 0 )
				return m_House.Sign.OriginalName;
...
//					}
				...
				case HouseGumpPageAOS.Storage:
				{
					...
					int curLockdowns = house.GetAosCurLockdowns(); EDIT */
					
					int maxLockdowns = house.GetAosMaxSecures();
					int curLockdowns = house.GetAosCurSecures( out fromSecures, out fromVendors, out fromLockdowns, out fromMovingCrate );

					int maxSecures = house.GetAosMaxLockdowns();
					int curSecures = house.GetAosCurLockdowns();
