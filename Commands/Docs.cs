	...
	public class Docs
	{
		...
		private static void GenerateStyles()
		{
			using( StreamWriter css = GetWriter( "docs/", "styles.css" ) )
			{
				...
				css.WriteLine( "td.ag {{ background-color: #{0:X6}; }}", Silver );
				css.WriteLine( "td.ve {{ background-color: #{0:X6}; }}", Obsidian );
				css.WriteLine( "td.va {{ background-color: #{0:X6}; }}", Steel );

				css.WriteLine( "td.cl {{ background-color: #{0:X6}; }}", Cloth );
				css.WriteLine( "td.pl {{ background-color: #{0:X6};  }}", Plain );
				css.WriteLine( "td.sp {{ background-color: #{0:X6}; }}", Core.AOS ? ThickAOS : ThickLBR );
				css.WriteLine( "td.ho {{ background-color: #{0:X6}; }}", Core.AOS ? BeastAOS : BeastLBR );
				css.WriteLine( "td.ba {{ background-color: #{0:X6}; }}", Core.AOS ? ScaledAOS : ScaledLBR );
			}
		}
		...
		private const int Silver = 0xDDAAAA;
		private const int Obsidian = 0x99CC77;
		private const int Steel = 0x88AABB;

		private const int Cloth = 0xDDDDDD;
		private const int Plain = 0xCCAA88;
		private const int ThickAOS = 0x99BBBB;
		private const int BeastAOS = 0xCC8888;
		private const int ScaledAOS = 0xAABBAA;
		private const int ThickLBR = 0xAA8833;
		private const int BeastLBR = 0xBBBBAA;
		private const int ScaledLBR = 0xCCAA88;
		...
		private static void DocumentBulkOrders()
		{
			using( StreamWriter html = GetWriter( "docs/bods/", "bod_smith_rewards.html" ) )
			{
				...
				sbod.RequireExceptional = false;
				for( BulkMaterialType mat = BulkMaterialType.None; mat <= BulkMaterialType.Steel; ++mat )
				{
					sbod.Material = mat;
					sbod.AmountMax = 10;
			...
			using( StreamWriter html = GetWriter( "docs/bods/", "bod_tailor_rewards.html" ) )
			{
				...
				for( BulkMaterialType mat = BulkMaterialType.None; mat <= BulkMaterialType.Scaled; ++mat )
				{
					if( mat >= BulkMaterialType.Copper && mat <= BulkMaterialType.Steel )
						continue;
		...
		private static void WriteTailorLBOD( StreamWriter html, string name, SmallBulkEntry[] entries, bool expandCloth, bool expandPlain )
		{
			...
			for( BulkMaterialType mat = BulkMaterialType.Thick; mat <= BulkMaterialType.Scaled; ++mat )
			{
				lbod.Material = mat;
				lbod.AmountMax = 10;
				DocumentTailorBOD( html, lbod.ComputeRewards( true ), "10, 15", lbod.Material, type );
		...
		private static void WriteTailorBODHeader( StreamWriter html, string title )
		{
			...
			html.WriteLine( "            <td width=\"25\" class=\"entry\"><center><img src=\"http://www.runuo.com/images/bodreward_runic_Thick.jpg\" alt=\"Runic Sewing Kit: Thick\"></center></td>" );
			html.WriteLine( "            <td width=\"25\" class=\"entry\"><center><img src=\"http://www.runuo.com/images/bodreward_runic_Beast.jpg\" alt=\"Runic Sewing Kit: Beast\"></center></td>" );
			html.WriteLine( "            <td width=\"25\" class=\"entry\"><center><img src=\"http://www.runuo.com/images/bodreward_runic_Scaled.jpg\" alt=\"Runic Sewing Kit: Scaled\"></center></td>" );
			html.WriteLine( "         </tr>" );
		}
		...
		private static void DocumentTailorBOD( StreamWriter html, List<Item> items, string amt, BulkMaterialType material, Type type )
		{
			...
			switch( material )
			{
				...
				case BulkMaterialType.Thick: style = "sp"; name = "Thick"; break;
				case BulkMaterialType.Beast: style = "ho"; name = "Beast"; break;
				case BulkMaterialType.Scaled: style = "ba"; name = "Scaled"; break;
			}
		...
		private static void DocumentSmithBOD( StreamWriter html, List<Item> items, string amt, BulkMaterialType material )
		{
			...
			switch( material )
			{
				...
				case BulkMaterialType.Silver: style = "ag"; name = "Silver"; break;
				case BulkMaterialType.Obsidian: style = "ve"; name = "Obsidian"; break;
				case BulkMaterialType.Steel: style = "va"; name = "Steel"; break;
			}
