...
{
	public abstract class BaseMount : BaseBreedableCreature, IMount
	{
		...
		public override bool ProtectedFromTermination{ get{ return (this.m_Rider != null || ProtectedHourFromTermination); } }
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if( Blessed )
			{
				from.SendMessage( "You cannot ride a stabled animal." );
				return;
			}
			...
			if ( from.InRange( this, 1 ) )
			{
				...
				if ( canAccess )
				{
                    if ( from is PlayerMobile )
                    {
                        ...
                        if ( pm.EscortPrisoner != null && pm.Feats.GetFeatLevel(FeatList.EscortPrisoner) < 3 )
                        {
                            pm.SendMessage( 60, "You cannot ride a mount while dragging a prisoner before reaching the third level of Escort Prisoner." );
                            return;
                        }

                        if( pm.DismountedTimer != null )
                        {
                            pm.SendMessage( 60, "You are still too dazed by the fall to ride again." );
                            return;
                        }
                    }
		...
		public static void Dismount( Mobile m )
		{
			...
			if( m is Mercenary )
			{
				Mercenary merc = m as Mercenary;
				merc.ActiveSpeed = 0.2;
				merc.PassiveSpeed = 0.4;
			}
		}
			...
			set
			{
				if ( m_Rider != value )
				{
					if ( value == null )
					{
						...
						if( m_Rider is Mercenary )
						{
							Mercenary merc = m_Rider as Mercenary;
							merc.ActiveSpeed = 0.2;
							merc.PassiveSpeed = 0.4;
						}
					...
					if( value is Mercenary )
					{
						Mercenary merc = value as Mercenary;
						merc.ActiveSpeed = 0.175;
						merc.PassiveSpeed = 0.350;
					}
