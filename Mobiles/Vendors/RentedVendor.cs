...
namespace Server.Mobiles
{
	...
	public class RentedVendor : PlayerVendor
	{
		...
		private int m_RentalCopper;

		private DateTime m_RentalExpireTime;
		private Timer m_RentalExpireTimer;

		public RentedVendor( Mobile owner, BaseHouse house, VendorRentalDuration duration, int rentalPrice, bool landlordRenew, int rentalCopper ) : base( owner, house )
		{
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int RentalCopper
		{
			get{ return m_RentalCopper; }
			set{ m_RentalCopper = value; }
		}
		...
		{
			if ( RentalCopper > 0 && House != null && House.IsAosRules )
			{
				if ( House.MovingCrate == null )
					House.MovingCrate = new MovingCrate( House );

				Banker.Deposit( House.MovingCrate, RentalCopper );
				RentalCopper = 0;
			}
		...
		private class CollectRentEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				if ( m_Vendor.RentalCopper > 0 )
				{
					int depositedCopper = Banker.DepositUpTo( from, m_Vendor.RentalCopper );
					m_Vendor.RentalCopper -= depositedCopper;
		...
		private class TerminateContractEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				from.SendLocalizedMessage( 1062503 ); // Enter the amount of copper you wish to offer the renter in exchange for immediate termination of this contract?
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (int) m_RentalCopper );
		...
		private class RentalExpireTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if ( m_Vendor.Renew && m_Vendor.HoldCopper >= renewalPrice )
				{
					m_Vendor.HoldCopper -= renewalPrice;
					m_Vendor.RentalCopper += renewalPrice;
