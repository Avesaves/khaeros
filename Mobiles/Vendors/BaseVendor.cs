...
namespace Server.Mobiles
{
	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
		...
		public ArrayList ArmorBuyInfo{ get{ return m_ArmorBuyInfo; } set{ m_ArmorBuyInfo = value; } }
		public ArrayList ArmorSellInfo{ get{ return m_ArmorSellInfo; } set{ m_ArmorSellInfo = value; } }
		...
		public virtual void UpdateBuyInfo()
		{
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			if ( dropped is SmallBOD || dropped is LargeBOD )
			{
				...
				if ( dropped is SmallBOD )
					((SmallBOD)dropped).GetRewards( out reward, out copper, out fame );
				else
					((LargeBOD)dropped).GetRewards( out reward, out copper, out fame );
				...
				if ( copper > 1000 )
					from.AddToBackpack( new BankCheck( copper ) );
				else if ( copper > 0 )
					from.AddToBackpack( new Copper( copper ) );
		...
		public GenericBuyInfo LookupDisplayObject( object obj )
		{
			IBuyItemInfo[] buyInfo = this.GetBuyInfo();
		...
		public virtual void ProcessSinglePurchase( BuyItemResponse buy, IBuyItemInfo bii, ArrayList validBuy, ref int controlSlots, ref bool fullPurchase, ref int totalCost )
		{
		...
		public virtual void ProcessValidPurchase( int amount, IBuyItemInfo bii, Mobile buyer, Container cont )
		{
			if ( amount > bii.Amount )
				amount = bii.Amount;
		...
		public virtual bool OnSellItems( Mobile seller, ArrayList list )
		{
			...
			foreach ( SellItemResponse resp in list )
			{
				...
				foreach( IShopSellInfo ssi in info )
				{
					if ( ssi.IsSellable( resp.Item ) )
					{
						...
						GiveCopper += ssi.GetSellPriceFor( resp.Item )*amount;
						break;
					}
				}
			}
			...
			//no cliloc for this?
			//SayTo( seller, true, "Thank you! I bought {0} item{1}. Here is your {2}gp.", Sold, (Sold > 1 ? "s" : ""), GiveCopper );
