...
{
	public class SouthernGuard : BaseKhaerosMobile, IRacialGuard, ISouthern
	{
        private DateTime m_speechInterval;
        private Nation guardNation;

		[Constructable]
        public SouthernGuard() : this( 0 )
        {
        }
        
		[Constructable]
		public SouthernGuard( int choice ) : base( Nation.Southern ) 
		{
			SetStr( 150 );
			SetDex( 75 );
			SetInt( 75 );
			...
			BaseKhaerosMobile.RandomGuardEquipment( this, Nation.Southern, choice );
                ...
                && (m.AccessLevel < AccessLevel.Counselor)) // if the seen mobile is not staff
            {
                if (attachment == null) // if there is no Criminal attachment
                {
                    if (((PlayerMobile)m).CriminalActivity) // Is the seen PlayerMobile participating in criminal activity?
                    {
                        ...
                        m.SendMessage("The Southerns have been alerted of your crimes!");

                        ((PlayerMobile)m).CriminalActivity = false;
                    }
                    else if (((PlayerMobile)m).Combatant is ISouthern)// Is the seen PlayerMobile fighting someone of the guard's race?
                    {
                        if (((PlayerMobile)m).Nation != guardNation) // Is the attacking PlayerMobile not of the guard's race?
                        {
                            XmlAttach.AttachTo(m, new XmlCriminal(guardNation));
                            m.SendMessage("The Southerns have been alerted of your assault!");
                        }
                    }
        ...
        private String RandomAttackSpeech()
        {
            ...
            if (DateTime.Now >= (m_speechInterval + TimeSpan.FromSeconds(30)))
            {
                ...
                {
                    case 1: { m_speechInterval = DateTime.Now; return "To victory!"; }
                    case 2: { m_speechInterval = DateTime.Now; return "I'll clear the way of you!"; }
                    case 3: { m_speechInterval = DateTime.Now; return "Stop right there, criminal scum!"; }
                    case 4: { m_speechInterval = DateTime.Now; return "With a red hand!"; }
                    case 5: { m_speechInterval = DateTime.Now; return "Your day has come!"; }
                    case 6: { m_speechInterval = DateTime.Now; return "Sons of the hounds, come here and get flesh!"; }
                    case 7: { m_speechInterval = DateTime.Now; return "There'll be no one to sing your name!"; }
                    case 8: { m_speechInterval = DateTime.Now; return "Your story ends here, blighter."; }
                }
            }
		...
		public SouthernGuard(Serial serial) : base(serial)
		{
		}
