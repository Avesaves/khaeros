...
{
	public class MhordulGuard : BaseKhaerosMobile, IRacialGuard, IMhordul
	{
        private DateTime m_speechInterval;
        private Nation guardNation;

		[Constructable]
        public MhordulGuard() : this( 0 )
        {
        }
        
		[Constructable]
		public MhordulGuard( int choice ) : base( Nation.Mhordul ) 
		{
			SetStr( 150 );
			SetDex( 75 );
			SetInt( 75 );
		...
		public MhordulGuard(Serial serial) : base(serial)
		{
		}
            ...
            {   
                if (attachment == null) // if there is no Criminal attachment
                {
                    if (((PlayerMobile)m).CriminalActivity) // Is the seen PlayerMobile participating in criminal activity?
                    {
                        ...
                        m.SendMessage("The Mhordul have been alerted of your crimes!");
