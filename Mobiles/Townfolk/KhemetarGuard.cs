...
{
	public class HaluarocGuard : BaseKhaerosMobile, IRacialGuard, IHaluaroc
	{
        private DateTime m_speechInterval;
        private Nation guardNation;

		[Constructable]
        public HaluarocGuard() : this( 0 )
        {
        }
        
		[Constructable]
		public HaluarocGuard( int choice ) : base( Nation.Haluaroc ) 
		{
			SetStr( 150 );
			SetDex( 75 );
			SetInt( 75 );
			...
			BaseKhaerosMobile.RandomGuardEquipment( this, Nation.Haluaroc, choice );
                ...
                && (m.AccessLevel < AccessLevel.Counselor)) // if the seen mobile is not staff
            {
                if (attachment == null) // if there is no Criminal attachment
                {
                    if (((PlayerMobile)m).CriminalActivity) // Is the seen PlayerMobile participating in criminal activity?
                    {
                        ...
                        m.SendMessage("The Haluaroc have been alerted of your crimes!");

                        ((PlayerMobile)m).CriminalActivity = false;
                    }
                    else if (((PlayerMobile)m).Combatant is IHaluaroc)// Is the seen PlayerMobile fighting someone of the guard's race?
                    {
                        if (((PlayerMobile)m).Nation != guardNation) // Is the attacking PlayerMobile not of the guard's race?
                        {
                            XmlAttach.AttachTo(m, new XmlCriminal(guardNation));
                            m.SendMessage("The Haluaroc have been alerted of your assault!");
                        }
                    }
        ...
        private String RandomAttackSpeech()
        {
            ...
            if (DateTime.Now >= (m_speechInterval + TimeSpan.FromSeconds(30)))
            {
                ...
                {
                    case 1: { m_speechInterval = DateTime.Now; return "Mah'tet wills it."; }
                    case 2: { m_speechInterval = DateTime.Now; return "Today is an auspicious day for death!"; }
                    case 3: { m_speechInterval = DateTime.Now; return "Stop right there, criminal scum!"; }
                    case 4: { m_speechInterval = DateTime.Now; return "I will sweep you away like the wind."; }
                    case 5: { m_speechInterval = DateTime.Now; return "The vultures take you!"; }
                    case 6: { m_speechInterval = DateTime.Now; return "Black sand!"; }
                    case 7: { m_speechInterval = DateTime.Now; return "You should know your place!"; }
                    case 8: { m_speechInterval = DateTime.Now; return "Ignorant sow!"; }
                }
            }
		...
		public HaluarocGuard(Serial serial) : base(serial)
		{
		}
