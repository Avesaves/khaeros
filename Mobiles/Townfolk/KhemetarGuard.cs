...
{
	public class KhemetarGuard : BaseKhaerosMobile, IRacialGuard, IKhemetar
	{
        private DateTime m_speechInterval;
        private Nation guardNation;

		[Constructable]
        public KhemetarGuard() : this( 0 )
        {
        }
        
		[Constructable]
		public KhemetarGuard( int choice ) : base( Nation.Khemetar ) 
		{
			SetStr( 150 );
			SetDex( 75 );
			SetInt( 75 );
			...
			BaseKhaerosMobile.RandomGuardEquipment( this, Nation.Khemetar, choice );
                ...
                && (m.AccessLevel < AccessLevel.Counselor)) // if the seen mobile is not staff
            {
                if (attachment == null) // if there is no Criminal attachment
                {
                    if (((PlayerMobile)m).CriminalActivity) // Is the seen PlayerMobile participating in criminal activity?
                    {
                        ...
                        m.SendMessage("The Khemetar have been alerted of your crimes!");

                        ((PlayerMobile)m).CriminalActivity = false;
                    }
                    else if (((PlayerMobile)m).Combatant is IKhemetar)// Is the seen PlayerMobile fighting someone of the guard's race?
                    {
                        if (((PlayerMobile)m).Nation != guardNation) // Is the attacking PlayerMobile not of the guard's race?
                        {
                            XmlAttach.AttachTo(m, new XmlCriminal(guardNation));
                            m.SendMessage("The Khemetar have been alerted of your assault!");
                        }
                    }
		...
		public KhemetarGuard(Serial serial) : base(serial)
		{
		}
