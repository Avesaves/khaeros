	...
	public class BaseShield : BaseArmor
	{
		...
		public override int OnHit( BaseWeapon weapon, int damage )
		{
			if( Core.AOS )
			{
				...
				{
					int wear = Utility.RandomMinMax( 1, 100 );

					if( wear > 95 && MaxHitPoints > 0 )
					{
						if( HitPoints > 0 )
							HitPoints--;
			...
			else
			{
				...
				*/
				...
				{
                    if (HealthAttachment.HasHealthAttachment(owner) && HealthAttachment.GetHA(owner).HasInjury(Injury.FracturedLeftArm))
                    {
