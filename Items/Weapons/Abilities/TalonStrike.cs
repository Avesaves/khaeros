...
namespace Server.Items
{
	...
	public class TalonStrike : WeaponAbility
	{
		...
		{
			if( GetSkill( from, SkillName.ExoticWeaponry ) < 50.0 )
			{
				from.SendLocalizedMessage( 1063352, "50" ); // You need ~1_SKILL_REQUIREMENT~ ExoticWeaponry skill to perform that attack!
				return false;
			}
		...
		public override void OnHit( Mobile attacker, Mobile defender, int damage )
		{
			...
			Timer t = new InternalTimer( defender, (int)(10.0 * (attacker.Skills[SkillName.ExoticWeaponry].Value - 50.0) / 70.0 + 5), attacker );	//5 - 15 damage
