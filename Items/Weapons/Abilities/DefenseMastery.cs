...
{
	/// <summary>
	/// Raises your physical resistance for a short time while lowering your ability to inflict damage. Requires Bushido or ExoticWeaponry skill.
	/// </summary>
	public class DefenseMastery : WeaponAbility
	{
		...
		{
			if( GetSkill( from, SkillName.ExoticWeaponry ) < 50.0  && GetSkill( from, SkillName.Leadership ) < 50.0 )
			{
				from.SendLocalizedMessage( 1063347, "50" ); // You need ~1_SKILL_REQUIREMENT~ Bushido or ExoticWeaponry skill to perform that attack!
				return false;
			}
		...
		public override void OnHit( Mobile attacker, Mobile defender, int damage )
		{
			...
			int modifier = (int)(30.0 * ((Math.Max( attacker.Skills[SkillName.Leadership].Value, attacker.Skills[SkillName.ExoticWeaponry].Value ) - 50.0) / 70.0));
