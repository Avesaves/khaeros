...
namespace Server.Items
{
	public class Fists : BaseMeleeWeapon
	{
		...
		public override SkillName DefSkill{ get{ return SkillName.UnarmedFighting; } }
		...
		{
			double wresValue = defender.Skills[SkillName.UnarmedFighting].Value;
			double anatValue = defender.Skills[SkillName.Anatomy].Value;
			double evalValue = defender.Skills[SkillName.Invocation].Value;
		...
		public override TimeSpan OnSwing( Mobile attacker, Mobile defender )
		{
			if ( attacker.StunReady )
			{
				...
				{
					if ( attacker.Skills[SkillName.Anatomy].Value >= 80.0 && attacker.Skills[SkillName.UnarmedFighting].Value >= 80.0 )
					{
						if ( attacker.Stam >= 15 )
						{
							attacker.Stam -= 15;
			...
			else if ( attacker.DisarmReady )
			{
				if ( attacker.CanBeginAction( typeof( Fists ) ) )
				{
					...
					{
						if ( attacker.Skills[SkillName.Craftsmanship].Value >= 80.0 && attacker.Skills[SkillName.UnarmedFighting].Value >= 80.0 )
						{
							if ( attacker.Stam >= 15 )
							{
								...
								else if ( CheckMove( attacker, SkillName.Craftsmanship ) )
								{
									StartMoveDelay( attacker );
		...
		/* UnarmedFighting moves */

		private static bool CheckMove( Mobile m, SkillName other )
		{
			double wresValue = m.Skills[SkillName.UnarmedFighting].Value;
		...
		private static void EventSink_DisarmRequest( DisarmRequestEventArgs e )
		{
			...
			double armsValue = m.Skills[SkillName.Craftsmanship].Value;
			double wresValue = m.Skills[SkillName.UnarmedFighting].Value;
