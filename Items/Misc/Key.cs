...
namespace Server.Items
{
	...
	public class Key : Item
	{
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( m_KeyVal == 0 )
				desc = "(blank)";
			else if ( (desc = Description) == null || (desc = desc.Trim()).Length <= 0 )
				desc = null;
			...
			if ( o.KeyValue == this.KeyValue )
			{
				...
				else
				{
					...
					if ( o is Item )
					{
						...
						if( item is Safe )
						{
							Safe safe = item as Safe;
							safe.RequiredSkill = 255;
							safe.LockLevel = 255;
							safe.MaxLockLevel = 255;
						}
		...
		private class CopyTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted is Key )
				{
					...
					else if ( from.CheckTargetSkill( SkillName.Tinkering, k, 0, 25.0 ) )
					{
				...
				else if( targeted is BaseDoor )
				{
					...
					BaseHouse house = BaseHouse.FindHouseAt( door );
					
					if( house == null )
						house = BaseHouse.FindHouseAt( new Point3D( door.X, door.Y + 1, door.Z ), door.Map, 1 );
					
					if( house == null )
						house = BaseHouse.FindHouseAt( new Point3D( door.X + 1, door.Y, door.Z ), door.Map, 1 );

					if( ( house != null && ( house.IsOwner( from ) || house.IsCoOwner( from ) ) ) || from.AccessLevel > AccessLevel.Player )
					{
						uint newlock = Key.RandomValue();
						m_Key.KeyValue = newlock;
						from.SendMessage( "You successfully changed the door's lock. If that door had any other doors linked to it, their locks have been changed too." );
						door.ChangeLock( newlock );
					}
					
					else if( house == null )
						from.SendMessage( "That door is not placed inside a house." );
					
					else
						from.SendMessage( "The house in which that door is placed does not belong to you." );
					
					return;
				}
				else if( targeted is BaseStorageContainer )
				{
					BaseStorageContainer box = (BaseStorageContainer)targeted;
					
					if( box.OwnersName == null )
						from.SendMessage( "You cannot change the lock of a storage box that is still for sale." );
					
					else if( box.OwnersName != from.Name )
						from.SendMessage( "The name on the storage box does not match yours." );
					
					else if( box.OwnersName == from.Name && box.Owner == from )
					{
						uint newlock = Key.RandomValue();
						m_Key.KeyValue = newlock;
						box.KeyValue = newlock;
						from.SendMessage( "You successfully changed the lock." );
					}
				...
				{
					LockableContainer chest = (LockableContainer)targeted;
					BaseHouse house = BaseHouse.FindHouseAt( chest );

					if( ( house != null && ( house.IsOwner( from ) || house.IsCoOwner( from ) ) ) || from.AccessLevel > AccessLevel.Player )
					{
						uint newlock = Key.RandomValue();
						m_Key.KeyValue = newlock;
						chest.KeyValue = newlock;
						from.SendMessage( "You successfully changed the container's lock." );
					}
					
					else if( house == null )
						from.SendMessage( "That container is not placed inside a house." );
					
					else
						from.SendMessage( "The house in which that container is placed does not belong to you." );
					
					return;
				}
				else
				{
					from.SendMessage( "That is not a key, a door or a lockable container." );
					return;
				}
