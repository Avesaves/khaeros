...
namespace Server.Items
{
	...
	public class SpikeTrap : BaseTrap
	{
		...
		public override void OnTrigger( Mobile from )
		{
			...
			bool fromDamaged = false;

			foreach ( Mobile mob in GetMobilesInRange( 0 ) )
			{
				if ( mob.Alive && !mob.IsDeadBondedPet )
				{
					Spells.SpellHelper.Damage( TimeSpan.FromTicks( 1 ), mob, mob, GetDamage() );
			...
			if( !fromDamaged && from.Alive && !from.IsDeadBondedPet )
				Spells.SpellHelper.Damage( TimeSpan.FromTicks( 1 ), from, from, GetDamage() );
