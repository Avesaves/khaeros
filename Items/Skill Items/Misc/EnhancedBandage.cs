...
{
	public class EnhancedBandage : Item
	{
		...
		public EnhancedBandage( int amount ) : base( 0xE21 )
		{
			Hue = 2125;
            Stackable = true;
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), Core.AOS ? 2 : 1 ) )
			{
				...
				from.SendLocalizedMessage( 500948 ); // Who will you use the Enhanced Bandages on?
		...
		{
			private EnhancedBandage m_EnhancedBandage;

			public InternalTarget( EnhancedBandage EnhancedBandage ) : base( 1, false, TargetFlags.Beneficial )
			{
			...
			{
				if ( m_EnhancedBandage.Deleted )
					return;

				if ( targeted is Mobile )
				{
					if ( from.InRange( m_EnhancedBandage.GetWorldLocation(), Core.AOS ? 2 : 1 ) )
					{
						if ( EnhancedBandageContext.BeginHeal( from, (Mobile)targeted ) != null )
						{
							int amount = m_EnhancedBandage.Amount;
							m_EnhancedBandage.Consume();	
						}
					}
					else
					{
				...
				{
					from.SendLocalizedMessage( 500970 ); // Enhanced Bandages can not be used on that.
				}
			}
		}
	}

	public class EnhancedBandageContext
	{
		...
		public EnhancedBandageContext( Mobile healer, Mobile patient, TimeSpan delay )
		{
			m_Healer = healer;
			m_Patient = patient;
		...
		public static EnhancedBandageContext GetContext( Mobile healer )
		{
			return (EnhancedBandageContext)m_Table[healer];
		}
		...
		public void EndHeal()
		{
			...
			{
				m_Healer.SendLocalizedMessage( 500969 ); // You finish applying the Enhanced Bandages.
			...
			else
			{
				...
				{
					healerNumber = 500969; // You finish applying the Enhanced Bandages.
					...
					{
						toHeal = 1;
						healerNumber = 500968; // You apply the Enhanced Bandages, but they barely help.
					}
		...
		{
			private EnhancedBandageContext m_Context;

			public InternalTimer( EnhancedBandageContext context, TimeSpan delay ) : base( delay )
			{
				m_Context = context;
		...
		public static EnhancedBandageContext BeginHeal( Mobile healer, Mobile patient )
		{
			...
			else if ( healer.CanBeBeneficial( patient, true, true ) )
			{
				...
				EnhancedBandageContext context = GetContext( healer );

				if ( context != null )
					context.StopHeal();

				context = new EnhancedBandageContext( healer, patient, TimeSpan.FromSeconds( seconds ) );
				...
				healer.SendLocalizedMessage( 500956 ); // You begin applying the Enhanced Bandages.
				return context;
			}
